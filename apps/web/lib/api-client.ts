...
export const auth = {
  ...
  current: () => request<{ pid: string; name: string; email: string }>("/api/auth/current"),
};
...
export const meals = {
  list: (date?: string) => request<Meal[]>(`/api/meals${date ? `?date=${date}` : ""}`),
  ...
  delete: (pid: string) => request<void>(`/api/meals/${pid}`, { method: "DELETE" }),
};
...
export const sleep = {
  list: (date?: string) => request<SleepRecord[]>(`/api/sleep${date ? `?date=${date}` : ""}`),
  create: (data: CreateSleepInput) =>
    request<SleepRecord>("/api/sleep", { method: "POST", body: JSON.stringify(data) }),
  delete: (pid: string) => request<void>(`/api/sleep/${pid}`, { method: "DELETE" }),
};
