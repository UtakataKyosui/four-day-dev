...
function ScoreCircle({ score, label }: { score: number; label: string }) {
  const color = score >= 80 ? 'text-green-500' : score >= 60 ? 'text-yellow-500' : 'text-destructive'
  return (
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-6 space-y-6">
      ...
      <div className="flex justify-around">
        ...
        {analysis.meal_score !== null && (
          <ScoreCircle score={analysis.meal_score} label="食事" />
        )}
        {analysis.sleep_score !== null && (
          <ScoreCircle score={analysis.sleep_score} label="睡眠" />
        )}
      </div>
