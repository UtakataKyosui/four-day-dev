...
import { fitbit as fitbitApi, sleep as sleepApi } from "@/lib/api-client";
import type { FitbitStatus, SleepRecord } from "@/lib/api-client";
  ...
  async function handleManualSubmit(e: React.FormEvent) {
    ...
    try {
      const record = await sleepApi.create({
        ...
        duration_minutes: Number.parseInt(manualForm.duration_minutes) || 0,
      });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-lg border border-border p-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          {fitbitStatus.connected && <SyncButton date={date} onSynced={fetchData} />}
        </div>
      </div>
            ...
            <div className="bg-card rounded-lg border border-border p-4">
              ...
              <form onSubmit={handleManualSubmit} className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label htmlFor="sleep-start" className="block text-xs font-medium mb-1">
                      就寝時刻
                    </label>
                    <input
                      id="sleep-start"
                      ...
                      onChange={(e) =>
                        setManualForm((p) => ({ ...p, sleep_start: e.target.value }))
                      }
                      required
                  ...
                  <div>
                    <label htmlFor="sleep-end" className="block text-xs font-medium mb-1">
                      起床時刻
                    </label>
                    <input
                      id="sleep-end"
                ...
                <div>
                  <label htmlFor="sleep-duration" className="block text-xs font-medium mb-1">
                    睡眠時間（分）
                  </label>
                  <input
                    id="sleep-duration"
                    ...
                    onChange={(e) =>
                      setManualForm((p) => ({ ...p, duration_minutes: e.target.value }))
                    }
                    required
                    min="0"
                    className="w-full px-2 py-1.5 text-sm border border-input rounded bg-background"
